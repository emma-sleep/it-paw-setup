...
if(!(Get-Command git).Path){
    if(!$WhatIfPreference){
        winget install git.git --disable-interactivity --no-warns
    }
...
if(!(Get-Command "ssh-keygen").Path){
    if(!$WhatIfPreference){
        winget install Microsoft.OpenSSH.Beta --disable-interactivity --no-warns
    }
    Write-Host "[Installed]" -ForegroundColor $Colors.Success
}else{
    Write-Host "[Skipped]" -ForegroundColor $Colors.Skipped
...
if(!(Get-Command pwsh).Path){
    if(!$WhatIfPreference){
        winget install Microsoft.PowerShell --disable-interactivity --no-warns
    }
    Write-Host "[Installed]" -ForegroundColor $Colors.Success
}else{
    Write-Host "[Skipped]" -ForegroundColor $Colors.Skipped
